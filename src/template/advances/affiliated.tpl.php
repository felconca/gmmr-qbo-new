<div class="d-flex justify-content-between align-items-center">
    <div>
        ...
        <h4 class="fw-semibold text-dark">Advances To Affiliated Company</h4>
    </div>
    ...
    <div class="d-flex align-items-center" style="gap:10px">
        ...
        <div class="d-flex align-items-center justify-content-end" style="gap:6px">
            <button
                ...
                ng-click="handleBookItems(selectedItems)"
                ng-disabled="selectedItems.length === 0 || isSending">
                <i class="ph-bold ph-share-fat me-1"></i>
                Book/Re-Book
            </button>
            <button
                class="btn btn-danger text-white"
                ng-click="handleUnBookedItems(selectedItems)"
                ng-disabled="selectedItems.length === 0 || isSending">
                <i class="ph-bold ph-trash text-white me-1"></i>
                Unbooked
            </button>
        </div>
    </div>
...
<div class="d-flex align-items-end justify-content-between mt-3" style="gap:6px">
    <div class="w-50">
        <div class="input-form-grp bg-white" style="width: 300px;">
            <input
                type="text"
                placeholder="Search"
                ng-model="search"
        ...
        <div class="d-flex align-items-end justify-content-end" style="gap: 6px;">
            ...
            <div class="filter-input" style="width: 150px;">
                ...
                <input
                    type="date"
                    class="input-form"
                    ng-model="filtered.endDate"
                    date-input
                    ng-disabled="isFiltering || isSending">
            </div>
            <button
                class="btn btn-theme-dark"
                ng-click="handleFilter(filtered, 'affiliated')"
                ng-disabled="isFiltering || isSending">
                <img ng-if="isFiltering" style="width:20px; height:20px" src="src/assets/img/loader_24x.svg" alt="Loading...">
...
<div class="table-cms mt-2" style="height: calc(100vh - 280px);">
    <table class="table align-middle mb-0 table-striped">
        ...
        <tbody>
            <tr ng-repeat="items in searched = (affiliatedList | filter: search) | limitTo:itemsPerPage:itemsPerPage*(currentPage-1) track by $index">
                <td>
                    <input
                        type="checkbox"
                        ng-disabled="isLoading || isSending"
                        class="form-check-input"
                        ng-model="items.selected"
                        ng-click="handleSelectItem(items)">
                </td>
                <td nowrap>{{items.trandate | date:"MM/dd/yy"}}</td>
                <td nowrap>
                    <a
                        class="link-anchor"
                        ng-click="showCreditModal(items.tranid)"
                        ng-disabled="isSending">{{items.cmid}}</a>
                </td>
                <td nowrap>
                    ...
                    <div class="d-flex align-items-center" ng-if="items.pxid > 0">
                        <a tooltip="Mapped to qbo customer" flow="right">
                            <i
                                class="ph-fill ph-seal-check text-success me-2"
                                ng-if="items.qbopx > 0"></i>
                        </a>
                        {{items.fname}}
                        <span ng-if="items.mname && items.mname.length > 0">
                            &nbsp;{{items.mname.substring(0, 1)}}.
                        </span>
                        {{items.lname}} {{items.suffix}}
                    </div>
                </td>
                <td nowrap>{{items.account}}</td>
                <td nowrap>
                    <span ng-if="items.ufname && items.ufname.length > 0">{{items.ufname.substring(0, 1)}}.</span>
                    {{items.ulname}}
                </td>
                ...
                <td nowrap>
                    <span ng-if="items.sent_date && items.sent_date !== ''">{{toISO(items.sent_date) | date:'MMM dd, yyyy hh:mm a'}}</span>
                    <span ng-if="!items.sent_date || items.sent_date === ''">—</span>
                </td>
                ...
                <td class="text-center">
                    <div class="dropdown">
                        <button
                            ng-disabled="selectedItems.length > 0 || isSending"
                            class="btn btn-sm btn-theme-dark dropdown-toggle"
                            type="button"
                        ...
                        <ul class="dropdown-menu btn-action">
                            <li>
                                <button
                                    ...
                                    type="button"
                                    ng-click="handleBookItems([items])">
                            ...
                            <li>
                                <button
                                    class="dropdown-item py-2 d-flex align-items-center"
                                    type="button"
                                    ng-click="findCredit(items.sent_id)"
                                    ng-disabled="!items.sent_id || items.sent_id === 0">
                            ...
                            <li>
                                <button
                                    class="dropdown-item py-2 d-flex align-items-center text-danger delete"
                                    type="button"
                                    ng-click="handleUnBookedItems([items])"
                                    ng-disabled="!items.sent_id || items.sent_id === 0">
...
<div class="d-flex align-items-center justify-content-between pt-3">
    ...
    <ul
        ...
        ng-change="changePage(affiliatedList)">
    </ul>
</div>