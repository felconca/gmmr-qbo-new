angular
  .module("app")
  .controller("nonpharmaCtrl", function ($scope, $state, $http, AuthService, SweetAlert2, $qbo, $uibModal, $filter) {
    const vm = $scope;
    const vs = $state;
    ...
    vm.handleInvoiceList = (filter) => {
      ...
      $http
        .get(
          `api/nonpharmacy/invoices?start_dt=${start_dt}&end_dt=${end_dt}&status=${filter.status}&isbooked=${filter.isBooked}`
        )
        .then((res) => {
      ...
      if (items.length > 0) {
        ...
        if (token) {
          let invoices = items.map((i) => ({
            ...
            docnumber: $qbo.status(1) + i.tranid,
            txndate: i.trandate,
            amount: i.netamount,
    ...
    vm.handleUnBookedItems = async (items) => {
      if (items.length > 0) {
        ...
        if (token) {
          ...
          $http
            .post("api/nonpharmacy/delete_invoice", { token: token, data: invoices })
            .then((res) => {
              Toasty.showToast(
                "Success",
