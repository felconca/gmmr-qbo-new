angular
  ...
  .controller(
    "professionalFeeCtrl",
    function ($scope, $state, $http, AuthService, SweetAlert2, $qbo, $uibModal, $filter) {
      const vm = $scope;
      const vs = $state;
      ...
      const PF_FILTER = JSON.parse(localStorage.getItem("pf-filter"));
      ...
      vm.handleInvoiceList = (filter) => {
        ...
        $http
          .get(`api/pf/invoices?start_dt=${start_dt}&end_dt=${end_dt}&isbooked=${filter.isBooked}`)
          .then((res) => {
      ...
      vm.handleFilter = (filtered) => {
        ...
        localStorage.setItem("pf-filter", JSON.stringify(filtered));
      };
      vm.handleInvoiceEdit = (id) => {
        $http
          .get(`api/pf/edit?id=${id}`)
          .then((res) => {
        ...
        if (items.length > 0) {
          ...
          if (token) {
            let invoices = items.map((i) => ({
              ...
              docnumber: $qbo.status(3) + i.tranid,
              txndate: i.trandate,
              amount: i.netamount,
            ...
            $http
              .post("api/pf/book_invoice", { token: token, data: invoices })
              .then((res) => {
                Toasty.showToast(
                  "Success",
      ...
      vm.handleUnBookedItems = async (items) => {
        if (items.length > 0) {
          ...
          if (token) {
            ...
            $http
              .post("api/pf/delete_invoice", { token: token, data: invoices })
              .then((res) => {
                Toasty.showToast(
                  "Success",
      ...
      vm.showInvoiceModal = (id) => {
        if (id > 0) {
          ...
          const $uibModalInstance = $uibModal.open({
            animation: true,
            templateUrl: "src/template/professional-fee/modal.tpl.php",
            size: "xl",
            scope: vm,
            backdrop: "static",
          });
      ...
      vm.findInvoice = async function (id) {
        let token = await AuthService.token("accesstoken");
        $http
          .post("api/pf/findInvoice", { token: token, id: id })
