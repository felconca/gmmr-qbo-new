...
class QueryBuilder
{
    ...
    public function UPSERT_BATCH($table, array $rows, array $updateColumns)
    {
        if (empty($table) || empty($rows)) return;
        ...
        if (!$result) {
            throw new \Exception("MySQL Upsert Batch Error: " . $this->conn->error . "\nQuery: $sql");
        }
    }
    /**
    ...
    public function RAW($sql, array $bindings = array())
    {
        ...
        // ── Strip leading whitespace and parentheses before detecting type
        $trimmed = ltrim($sql, " \t\n\r(");
        $type    = strtoupper(strtok($trimmed, " \t\n\r"));
