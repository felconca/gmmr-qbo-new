...
class InvoicesService
{
    ...
    public function update(array $data, $db)
    {
        ...
        try {
            foreach ($data as $row) {
                ...
                $amount = $row["amount"];
                $status = $row["status"];
                $qboid  = isset($row["qboid"]) ? $row["qboid"] : null;
    ...
    /**
     *  @param batching update
     */
    public function pf_line_batch(array $ids)
    {
        if (empty($ids)) return [];
        ...
        $items = $this->conn->wgcentralsupply()
            ...
            ->LEFTJOIN("ipadrbg.lkup_centers cx", "cx.centerRID = pd.centerRIDpbr")
            ->WHERE("pd.DisLineCanceled = 0")
            ->WHERE_IN("pd.TranRID", $ids) // ← only change
            ->get();

        return $items ? $items : [];
    }
    public function credit_line_batch(array $ids)
    {
        $items = $this->conn->wgcentralsupply()
            ->SELECT(
                "pd.TranRID AS tranid,
