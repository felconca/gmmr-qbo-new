...
class DebitMemoController extends Rest
{
    ...
    // qbo functions
    public function book_debit($request, $response, $params)
    {
        try {
            ...
            foreach ($debits as $row) {
                ...
                try {
                    ...
                    $line = $this->line_debit($row["tranid"]);
    ...
    public function delete_debit($request, $response, $params)
    {
        $invoiceService = new InvoicesService($this->db);
    ...
    public function find_debit($request, $response, $params)
    {
        $token = $request["token"];
        $id = $request["id"];
        $qbo = new QboEntityService($this->db, $this->companyId);
        $details = $qbo->details($id, $token, "Invoice");
        return $response($details, $details['status']);
    }
    private function line_debit($id)
    {
        $invoiceService = new InvoicesService($this->db);
