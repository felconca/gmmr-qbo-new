...
class SaleReturnController extends Rest
{
    ...
    {
        return $response(['message' => 'SaleReturnController index'], 200);
    }

    public function pharmacy($request, $response, $params)
    {
        try {
    ...
    public function nonpharma($request, $response, $params)
    {
        try {
            $input = $request->validate([
                "start_dt" => "required|date",
    ...
    public function edit($request, $response, $params)
    {
        try {
            ...
            $id = $input['id'];
            $conn = $input['database'] === "wgfinance" ? "wgfinance" : "wgcentralsupply";
            $returns = $this->db->$conn()
                ->SELECT([
                    "p.TranRID AS tranid",
                    "p.PxRID AS pxid",
                    "p.TranDate AS trandate",
            ...
            if ($returns) {
                $details = $conn == "wgfinance" ? $invoiceService->pharmacy_line($id) : $invoiceService->nonpharma_line($id);
                if ($details) {
                    $data = [
                        "cm" => $returns,
                        "details" => $details
    ...
    // qbo functions
    public function book_returns($request, $response, $params)
    {
        try {
            ...
            foreach ($credits as $row) {
                ...
                try {
                    ...
                    $line = $db == "wgfinance" ? $this->line_pharma($row["tranid"]) : $this->line_nonpharma($row["tranid"]);
                    ...
                    //Always update DB
                    $invoiceService->update($updateData, $db);
    ...
    public function delete_returns($request, $response, $params)
    {
        $invoiceService = new InvoicesService($this->db);
    ...
    public function find_returns($request, $response, $params)
    {
        $token = $request["token"];
        $id = $request["id"];
        $qbo = new QboEntityService($this->db, $this->companyId);
    ...
    private function line_pharma($id)
    {
        $invoiceService = new InvoicesService($this->db);
        ...
        foreach ($details as $list) {
            ...
            $price = isset($list["price"]) ? abs($list["price"]) : 0;
            $gross = isset($list["gross"]) ? abs($list["gross"]) : 0;
            $discount = isset($list["discount"]) ? abs($list["discount"]) : 0;
            $ldiscount = isset($list["ldiscount"]) ? abs($list["ldiscount"]) : 0;
            ...
            $unitPrice = (isset($list["vat"]) && $list["vat"] > 0)
                ? ($price ? abs($price / 1.12) : 0)
                : $price;
            $taxInclusiveAmt = $gross;
            $discountAmt = $ldiscount + $discount;
            $amount = (isset($list["vat"]) && $list["vat"] > 0)
                ? ($gross ? abs($gross / 1.12) : 0)
                : $gross;
            ...
            // Add to totals for subtotal and discounts (use absolute values)
            $grossTotal += $gross;
            $discountTotal += $discount;
            $ldiscountTotal += $ldiscount;
        }
        ...
        // Add discount line
        $discountSum = abs($discountTotal) + abs($ldiscountTotal);
        $lines[] = [
            ...
            "SalesItemLineDetail" => [
                ...
                "Qty" => 0,
                "UnitPrice" => 0 - abs($discountSum),
                "ItemRef" => ["value" => $qbo->discount(), "name" => "Discount"],
            ],
            "LineNum" => count($details) + 2,
            "Amount" => 0 - abs($discountSum),
        ];

        return $lines;
    }
    private function line_nonpharma($id)
    {
        $invoiceService = new InvoicesService($this->db);
        ...
        foreach ($details as $list) {
            ...
            // Use absolute values for all money and quantity related fields
            $qty = isset($list["qty"]) ? abs($list["qty"]) : 1;
            $price = isset($list["price"]) ? abs($list["price"]) : 0;
            $gross = isset($list["gross"]) ? abs($list["gross"]) : 0;
            $discount = isset($list["discount"]) ? abs($list["discount"]) : 0;
            ...
            // Add to totals for subtotal and discounts (these should also use abs)
            $grossTotal += $gross;
            $discountTotal += $discount;
            $ldiscountTotal += $ldiscount;
        }

        // Add subtotal line (show abs value)
        $lines[] = [
            "LineNum" => count($details) + 1,
            "Description" => "Subtotal: PHP" . abs($grossTotal),
            "DetailType" => "DescriptionOnly",
